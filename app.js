...
function render() {
  ...
  app.innerHTML = `
    ...
    ${state.showLibraryModal ? renderLibraryModal() : ""}
  `;
...
function renderFoodTab() {
  ...
  return `
    ...
    <button class="card" data-action="open-library-modal" style="width:100%;display:flex;align-items:center;justify-content:space-between;text-align:right">
      <span class="section-title">${ICONS.book} ספר מוצרים
        <span class="tiny muted">${state.savedFoods.length > 0 ? `(${state.savedFoods.length})` : ""}</span>
      </span>
...
function renderLibraryModal() {
  return `
    <div class="modal-backdrop" data-action="close-library-modal">
      <div class="modal-sheet" data-action="stop">
        <div class="row center" style="justify-content:space-between;margin-bottom:12px">
          <p class="section-title" style="font-size:16px">${ICONS.book} ספר מוצרים</p>
          <button class="icon-btn" data-action="close-library-modal">${ICONS.x}</button>
        </div>
        <div class="modal-scroll">
          <button class="btn btn-full" data-action="toggle-library-form" style="background:var(--food-soft);color:var(--food);margin-bottom:12px">${ICONS.plus} מוצר חדש</button>
...
function onAppClick(e) {
  ...
  switch (action) {
    ...
    case "open-library-modal": state.showLibraryModal = true; render(); break;
    case "close-library-modal": state.showLibraryModal = false; render(); break;
